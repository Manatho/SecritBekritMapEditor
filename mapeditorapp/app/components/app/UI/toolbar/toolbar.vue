<template>
  <div class="toolbar">
    ...
    <template v-for="tool in tools">
      <toolbutton v-bind:tooldata="tool" v-bind:key="tool.id"></toolbutton>
      <hr class="spacer">
    </template>
    <template v-for="(option,index) in Controller.tool.options">
      <tooloption v-bind:option="option" v-bind:key="option.id"></tooloption>
...
export default {
  ...
  mounted() {
    let refs = this.$refs;
    Controller.subscribe(ControllerEvents.Event_PNG_Data_Changed, function(
      event
    ) {
